...
def make_solver(save_name, train_nets, test_nets, **kwargs):
  ...
  write_txt = open('prototxts/' + save_name, 'w')
  write_txt.writelines('train_net: "prototxts/%s"\n' %train_nets)
  for tn in test_nets:
    write_txt.writelines('test_net: "prototxts/%s"\n' %tn)
...
def make_bash_script(save_bash, solver, weights=None, gpu=2):
  ...
  if weights:
    write_txt.writelines("../../build/tools/caffe train -solver prototxts/%s -weights %s -gpu %d" %(solver, weights, gpu))
  else:
    write_txt.writelines("../../build/tools/caffe train -solver prototxts/%s -gpu %d" %(solver, gpu))
  write_txt.close()  
